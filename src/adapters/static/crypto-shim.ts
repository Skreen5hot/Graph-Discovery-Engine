/**
 * Browser shim for node:crypto.createHash.
 * Uses the Web Crypto API (SubtleCrypto) via a synchronous polyfill
 * that matches the Node.js createHash("sha256").update(str).digest("hex") pattern.
 */
...
class BrowserHash {
  ...
  digest(encoding: "hex"): string {
    ...
    // The Web Crypto API is async, so we use a pure-JS fallback
    // that produces the same SHA-256 output.
